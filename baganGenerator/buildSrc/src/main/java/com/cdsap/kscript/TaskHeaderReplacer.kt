...
abstract class TaskHeaderReplacer : DefaultTask() {
    ...
    private fun createOutputFolder() {
        ...
        val kscriptOutput = "$dir/baganGenerator"
        val dirKscript = File(kscriptOutput)
        val dirGenerator = File("$kscriptOutput/generator")
        val dirExperiments = File("$kscriptOutput/experiments")
        if (!dirKscript.exists()) {
            dirKscript.mkdir()
        }
        if (!dirGenerator.exists()) {
            dirGenerator.mkdir()
        }
        if (!dirExperiments.exists()) {
            dirExperiments.mkdir()
        }
    }
